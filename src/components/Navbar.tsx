      ...
      <div className="bg-[#1a2744] text-white text-sm py-2 hidden md:block">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center gap-6">
            ...
            <a href={getWhatsAppUrl(HOTEL_INFO.whatsapp, 'Hello! I would like to inquire about room availability.')} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-[#c9a96e] transition-colors">
              ...
              {publicLinks.map((link) => (
                <Link
                  ...
                  className={`text-sm font-medium transition-colors hover:text-[#c9a96e] ${
                    pathname === link.href ? 'text-[#c9a96e]' : 'text-gray-700'
                  }`}
                >
                  {link.label}
                </Link>
          ...
          <div className="lg:hidden border-t bg-white">
            <div className="px-4 py-3 space-y-2">
              ...
              <div className="flex gap-2 mt-3">
                ...
                <a href={getWhatsAppUrl(HOTEL_INFO.whatsapp, 'Hello! I would like to inquire about room availability.')} target="_blank" rel="noopener noreferrer" className="btn-whatsapp text-sm flex-1 !py-2">
                  WhatsApp
                </a>
              </div>
            </div>
...
function AdminNavbar({ user, onLogout }: { user: User | null; onLogout: () => void }) {
  ...
  return (
    <nav className="bg-[#1a2744] shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          <Link href="/admin" className="flex items-center gap-2">
            <span className="text-white font-bold text-lg">HOTEL RIYANSH</span>
            <span className="bg-[#c9a96e] text-white text-xs px-2 py-0.5 rounded font-medium">Admin</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <Link href="/" className="text-sm text-gray-300 hover:text-white">View Site</Link>
      ...
      {isOpen && (
        <div className="md:hidden border-t border-gray-600 bg-[#1a2744]">
          <div className="px-4 py-3 space-y-2">
            {adminLinks.map((link) => (
              <Link
                key={link.href}
                ...
                onClick={() => setIsOpen(false)}
                className={`block py-2 text-sm font-medium ${
                  pathname === link.href ? 'text-[#c9a96e]' : 'text-gray-300'
                }`}
              >
                {link.label}
              </Link>
            ...
            <Link href="/" onClick={() => setIsOpen(false)} className="block py-2 text-sm text-gray-300">View Site</Link>
            <button onClick={() => { onLogout(); setIsOpen(false); }} className="block py-2 text-sm text-gray-300">Logout</button>
          </div>
        </div>
      )}
    </nav>
